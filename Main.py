...
for dmMass in dmMasses:
	data = rp.readColumnFile(dataFolder+"nuyield_ptr_output_bb_"+str(dmMass)+".dat", header=1)
...
ax = rp.plotInit(xAx=r"Neutrino Energy [GeV]", yAx=r"Neutrino Yields $[m^2 \times GeV \times annihilation]^{-1}$", plotTitle=r"nuyield output for neutrinos bb annihilation", grid=True, log=True)#, xLimits=[0,10], yLimits=[-0.1,1.1])
...
if gl.plot:
	if gl.save:
		rp.plotOutput(savefigname=plotFolder+"nuyieldoutput_neutrinos_bb.pdf",resolution=500)
	else:
		rp.plotOutput()

ax = rp.plotInit(xAx=r"Antineutrino Energy [GeV]", yAx=r"Antineutrino Yields $[m^2 \times GeV \times annihilation]^{-1}$", plotTitle=r"nuyield output for antineutrinos bb annihilation", grid=True, log=True)#, xLimits=[0,10], yLimits=[-0.1,1.1])
for i in range(len(colours)):
...
if gl.plot:
	if gl.save:
		rp.plotOutput(savefigname=plotFolder+"nuyieldoutput_antineutrinos_bb.pdf",resolution=500)
	else:
		rp.plotOutput()

ax = rp.plotInit(xAx=r"Neutrino Energy [GeV]", yAx=r"Neutrino Yields $[m^2 \times GeV \times annihilation]^{-1}$", plotTitle=r"nuyield output for neutrinos and antineutrinos bb annihilation", grid=True, log=True)#, xLimits=[0,10], yLimits=[-0.1,1.1])
for i in range(len(colours)):
	rp.plotData(ax, eners[i], nus[i], eXs=0, eYs=0, dataLabel=r"Nu for DM mass of "+str(dmMasses[i])+"GeV", colour=colours[i], lines=False, scale=0.5, marker='x')
	rp.plotData(ax, eners[i], antinus[i], eXs=0, eYs=0, dataLabel=r"Antinu for DM mass of "+str(dmMasses[i])+"GeV", colour=colours[i], lines=False, scale=0.5, marker='s')
if gl.plot:
	if gl.save:
		rp.plotOutput(savefigname=plotFolder+"nuyieldoutput_all_bb.pdf",resolution=500)
	else:
		rp.plotOutput()
